...
/**
 * Builder that buffers sorted-map entries until the JDK bulk-construction fast path is proven
 * safe. If the stream order is invalid, the builder falls back to normal tree insertion so the
 * resulting map keeps the same semantics as the old incremental read path.
 */
@SuppressWarnings({"rawtypes", "unchecked"})
...
final class ContainerTransfer {
  ...
  static <T extends Collection> Collection readCollection(
    ...
    return readCollection(type, construction, registrar, initializer, 0, false, 0);
  }
  ...
  static <T extends Collection> Collection readCollection(
    ...
    if (construction.getKind() == ContainerConstructors.Kind.DIRECT) {
      ...
      return maybeWrapSortedSetReadTarget(
          collection, numElements, bulkReadEnabled, bulkReadBufferLimitBytes);
    }
  ...
  static <T extends Map> Map readMap(
      ...
      Consumer<T> registrar,
      Consumer<T> initializer) {
    return readMap(type, construction, registrar, initializer, 0, false, 0);
  }
  ...
  static <T extends Map> Map readMap(
    ...
    if (construction.getKind() == ContainerConstructors.Kind.DIRECT) {
      ...
      return maybeWrapSortedMapReadTarget(
          map, numElements, bulkReadEnabled, bulkReadBufferLimitBytes);
    }
    ...
    Map rootMap =
        maybeWrapSortedMapReadTarget(
            rootTransferConstruction.newRootMap(),
            numElements,
            bulkReadEnabled,
            bulkReadBufferLimitBytes);
  ...
  private static Collection maybeWrapSortedSetReadTarget(
  ...
  private static Map maybeWrapSortedMapReadTarget(
      Map map, int numElements, boolean bulkReadEnabled, int bulkReadBufferLimitBytes) {
    if (bulkReadEnabled
